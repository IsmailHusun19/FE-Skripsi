...
const AddMatkulDosen = ({
  ...
  const handleMateri = async (e) => {
    ...
    if (editMataKuliah) {
      try {
        const editMatkul = await putMataKuliah(idMataKuliah, name);
        setOpen(false);
        handleClose(false);
        getMataKuliah();
        editDataMateri();
      ...
      if (dataJudul && name.trim() !== "") {
        ...
        try {
          ...
          if (metode === "post") {
            navigate(
              `/tambahmateri/${idMataKuliah}/${response.data.materi.id}`
            );
            return;
          }
          getData();
          ...
          >
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <h1 className="font-semibold mb-4">
                {Object.keys(handleEditMateri).length !== 0
                  ? "Edit Materi Mata Kuliah"
                  : "Buat Materi Mata Kuliah"}
              </h1>
