import { getAllMataKuliahAdmin, deleteMatkulDosen, } from "../../config/FetchingData";
...
const MataKuliah = () => {
  ...
  const [idMatkulDelete, setIdMatkulDelete] = useState(null);
  ...
  const confirmDelete = (id) => {
    setIdMatkulDelete(id);
    setDeleteMatkul(true);
  };

  const handleDeleteMatkul = async () => {
    if (!idMatkulDelete || !deleteMatkul) return;
    ...
    try {
      if (user.role === "Mahasiswa") {
        ...
        Confirm.show(
          ...
          "No",
          async () => {
            await deleteMatkulDosen(idMatkulDelete);
  ...
  useEffect(() => {
    if (deleteMatkul) handleDeleteMatkul();
  }, [deleteMatkul]);
                ...
                <section className="dark:bg-gray-900 w-full">
                  <div className="mx-auto max-w-screen-xl px-4 lg:px-12">
                    <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden my-10">
                      <h1 className="font-semibold text-xl px-5">
                        Mata Kuliah
                      </h1>
                          ...
                          <tbody>
                            {matkul
                              ...
                              .map((matkul, index) => (
                                <tr
                                  key={matkul.id}
                                  className="border-b dark:border-gray-700 text-gray-800"
                                >
                                  ...
                                  <FontAwesomeIcon
                                                ...
                                                onClick={() =>
                                                  navigate(
                                                    `/admin/matakuliah/materi/${matkul.id}`
                                                  )
                                                }
                                              />
                                    <FontAwesomeIcon
                                      ...
                                      icon={faTrash}
                                      onClick={() => confirmDelete(matkul.id)}
                                    />
                                  </td>
                                </tr>
                              ))}
