...
import { getDosenById, putDosen } from "../../config/FetchingData";
import { Notify } from 'notiflix/build/notiflix-notify-aio';

const EditDosen = () => {
  const { expanded } = useContext(SidebarContext);
  const [dosen, setDosen] = useState([]);
  const {idDosen} = useParams();
  ...
  const getDataDosen = async () => {
    setLoading(true);
    try {
      const getData = await getDosenById(idDosen);
      setDosen(getData.data.data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const put = await putDosen(data, idDosen);
