...
import { getMahasiswaById, putMahasiswa } from "../../config/FetchingData";
...
const EditMahasiswa = () => {
  ...
  const getDataMahasiswa = async () => {
    setLoading(true);
    try {
      const getData = await getMahasiswaById(idMahasiswa);
      console.log(getData)
      const dosen = getData.data.data;
      setNama(dosen.nama);
      setEmail(dosen.email);
      setNomorinduk(dosen.nomorinduk);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const put = await putMahasiswa(data, idMahasiswa);
      Notify.success('Berhasil mengedit');
    } catch (error) {
...
`}
          >
            <form className="w-[90%] m-auto mt-10" onSubmit={handleSubmit}>
              <h1 className="font-semibold text-xl mb-5">Edit Mahasiswa</h1>
              <div className="mb-6">
                <div>
                  <label
                    htmlFor="first_name"
